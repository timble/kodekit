...
!function( $ ) {
	...
	var Datepicker = function(element, options) {
		...
		this.format = DPGlobal.parseFormat(options.format||this.element.data('date-format')||'mm/dd/yyyy hh:ii:ss');
                this.isInline = false;
        ...
        if(options.parentEl){
            this.parentEl = $(options.parentEl).css({position:'relative'});
        } else {
            this.parentEl = 'body';
        }
        this.picker = $(DPGlobal.template)
                            .appendTo(this.isInline ? this.element : this.parentEl)
                            .on({
	...
	Datepicker.prototype = {
		...
		update: function(){
            ...
            if(arguments && arguments.length && (typeof arguments[0] === 'string' || arguments[0] instanceof Date)) {
                ...
                date = this.isInput ? this.element.prop('value') : this.element.find('input').prop('value');
            }
		...
		fill: function() {
			...
			this.updateNavArrows();
			this.fillMonths();
			var prevMonth = UTCDate(year, month-1, 28,this.date.getUTCHours(),this.date.getUTCMinutes(),this.date.getUTCSeconds(),0),
				...
				switch(target[0].nodeName.toLowerCase()) {
					...
					case 'td':
						if (target.is('.day') && !target.is('.disabled')){
							...
							this._setDate(UTCDate(year, month, day,this.date.getUTCHours(),this.date.getUTCMinutes(),this.date.getUTCSeconds(),0));
						}
						break;
				}
			}
		...
		keydown: function(e){
			...
			if (dateChanged || !this.keyboardNavigation){
				this.element.trigger({
					type: 'changeDate',
					date: this.date
				});
	...
	var DPGlobal = {
		...
		validParts: /ss?|ii?|hh?|dd?|mm?|MM?|yy(?:yy)?/g,
		nonpunctuation: /[^ -\/:-@\[-`{-~\t\n\r]+/g,
		...
		formatDate: function(date, format, language){
			var val = {
			    ...
			    i: (date.getUTCMinutes() < 10 ? '0' : '') + date.getUTCMinutes(),
