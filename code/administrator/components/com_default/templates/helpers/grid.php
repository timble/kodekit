...
class ComDefaultTemplateHelperGrid extends KTemplateHelperGrid
{
    public function publish($config = array())
    {
        ...
        if (version_compare(JVERSION, '1.6', '>=')) {
            ...
            $alt 	= $config->row->{$config->field} ? $this->translate('JPUBLISHED') : $this->translate('JUNPUBLISHED');
            $text 	= $config->row->{$config->field} ? $this->translate('JLIB_HTML_UNPUBLISH_ITEM') : $this->translate('JLIB_HTML_PUBLISH_ITEM');
    ...
    public function order($config = array())
    {
        ...
        if (version_compare(JVERSION, '1.6', '>=')) {
            ...
            if ($config->row->{$config->field} > 1) {
                $html .= sprintf($tmpl, $this->translate('JLIB_HTML_MOVE_UP'), $updata, 'uparrow');
            }
    
            $html .= $config->row->{$config->field};
    
            if ($config->row->{$config->field} != $config->total) {
                $html .= sprintf($tmpl, $this->translate('JLIB_HTML_MOVE_DOWN'), $downdata, 'downarrow');
            }
        } else {
