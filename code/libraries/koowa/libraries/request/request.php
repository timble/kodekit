...
class KRequest
{
    ...
    public static function get($identifier, $filter, $default = null)
    {
        ...
        if(!($filter instanceof KFilterInterface)) {
            $filter = KObjectManager::getObject('koowa:filter.factory')->instantiate($filter);
        }
    ...
    public static function referrer($isInternal = true)
    {
        if(!isset(self::$_referrer))
        {
            ...
            {
                self::$_referrer = KObjectManager::getObject('koowa:http.url', array('url' => $referrer));
            }
        }

        if($isInternal)
        {
        	if(!KObjectManager::getObject('koowa:filter.internalurl')->validate((string)self::$_referrer)) {
        		return null;
        	}
        }
    ...
    public static function url()
    {
        if(!isset(self::$_url))
        {
            ...
            $url = KObjectManager::getObject('koowa:filter.url')->sanitize($url);

            // Create the URI object
            self::$_url = KObjectManager::getObject('koowa:http.url', array('url' => $url));
    ...
    public static function base()
    {
        if(!isset(self::$_base))
        {
            ...
            // Sanitize the url since we can't trust the server var
            $path = KObjectManager::getObject('koowa:filter.url')->sanitize($path);

            self::$_base = KObjectManager::getObject('koowa:http.url', array('url' => $path));
        }
    ...
    public static function root($path = null)
    {
        if(!is_null($path))
        {
            if(!$path instanceof KHttpUrl) {
                $path = KObjectManager::getObject('koowa:http.url', array('url' => $path));
            }
